...
struct pokemon *encontrar_pokemon_buscado(abb_t *pokedex,
					  struct pokemon *pokemon)
{
	...
	pokemon = abb_obtener(pokedex, pokemon);
	if (!pokemon)
		return NULL;
...
bool imprimir_pokemones_por_pantalla(void *elemento, void *ctx)
{
	if (!elemento)
		return false;
...
int main(int argc, char *argv[])
{
	...
	while (!es_correcto) {
		...
		if (!scanf("%i", &opcion)) {
			...
			cerrar_archivo_csv(archivo);
			abb_destruir_todo(pokedex, destruir_pokemon);
			return ERROR;
		}
